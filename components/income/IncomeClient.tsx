...
export function IncomeClient({ householdId }: Props) {
  ...
  const incomeWorkspaces = workspaces.filter(w => w.type === 'income')
  const workspaceId = activeWorkspaceId ?? incomeWorkspaces[0]?.id ?? null

  const { data: entries = [] } = useQuery({
    queryKey: ['income', workspaceId, monthKey],
    queryFn: () => workspaceId ? getIncomeEntries(supabase, workspaceId, monthKey) : [],
    enabled: !!workspaceId,
  })
  ...
  async function handleGenerate() {
    ...
    const count = await generateRecurringForMonth(supabase, workspaceId, householdId, monthKey)
    queryClient.invalidateQueries({ queryKey: ['income', workspaceId, monthKey] })
  ...
  if (!incomeWorkspaces.length) {
    return <EmptyState icon={DollarSign} title="Nenhum workspace de receitas" description="Crie um workspace de receitas em Configurações." />
  }
        ...
        {incomeWorkspaces.map(ws => (
          <button
            ...
            className={`flex items-center gap-2 rounded-lg px-3 py-1.5 text-sm transition-colors ${
              (activeWorkspaceId ?? incomeWorkspaces[0]?.id) === ws.id
