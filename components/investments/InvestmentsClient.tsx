...
export function InvestmentsClient({ householdId }: Props) {
  ...
  const workspaceId = activeWorkspaceId ?? investmentWorkspaces[0]?.id ?? null

  const { data: investments = [] } = useQuery({
    queryKey: ['investments', workspaceId],
    queryFn: () => workspaceId ? getInvestments(supabase, workspaceId) : [],
    enabled: !!workspaceId,
  })
  ...
  async function handleAddInvestment(e: React.FormEvent) {
    ...
    queryClient.invalidateQueries({ queryKey: ['all-investments', householdId] })
  ...
  if (!investmentWorkspaces.length) {
    return (
      <EmptyState
        icon={TrendingUp}
        title="Nenhum workspace de investimentos"
        description="Crie um workspace de investimentos em Configurações."
      />
    )
  }
        ...
        {investmentWorkspaces.map(ws => (
          <button
            ...
            className={`flex items-center gap-2 rounded-lg px-3 py-1.5 text-sm transition-colors ${
              (activeWorkspaceId ?? investmentWorkspaces[0]?.id) === ws.id
