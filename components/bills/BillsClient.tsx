...
export function BillsClient({ householdId }: Props) {
  ...
  const billsWorkspaces = workspaces.filter(w => w.type === 'bills')
  const workspaceId = activeWorkspaceId ?? billsWorkspaces[0]?.id ?? null

  const { data: record, isLoading } = useQuery({
    queryKey: ['bills', workspaceId, monthKey],
    queryFn: () => workspaceId
      ? getOrCreateMonthRecord(supabase, workspaceId, householdId, monthKey)
      : null,
    enabled: !!workspaceId,
  })

  const { data: templates = [] } = useQuery({
    queryKey: ['bill-templates', workspaceId],
    queryFn: () => workspaceId ? getBills(supabase, workspaceId) : [],
    enabled: !!workspaceId,
  })
  ...
  async function handleToggleStatus(entryId: string, current: 'pending' | 'paid' | 'overdue') {
    ...
    queryClient.invalidateQueries({ queryKey: ['bills', workspaceId, monthKey] })
  }
  ...
  async function handleGenerate() {
    ...
    await generateMonthFromTemplates(supabase, workspaceId, householdId, monthKey)
    queryClient.invalidateQueries({ queryKey: ['bills', workspaceId, monthKey] })
  ...
  async function handleAddTemplate(e: React.FormEvent) {
    ...
    queryClient.invalidateQueries({ queryKey: ['bill-templates', workspaceId] })
  ...
  if (!billsWorkspaces.length) {
    return (
      <EmptyState
        icon={Receipt}
        title="Nenhum workspace de contas"
        description="Crie um workspace de contas em Configurações para começar."
      />
    )
  }
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex gap-2 mb-4">
        {billsWorkspaces.map(ws => (
          <button
            key={ws.id}
            onClick={() => setActiveWorkspaceId(ws.id)}
            className={`flex items-center gap-2 rounded-lg px-3 py-1.5 text-sm transition-colors ${
              (activeWorkspaceId ?? billsWorkspaces[0]?.id) === ws.id
